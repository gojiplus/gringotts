...
def charge_user(db: Session, user: models.User, cost: int, endpoint: str | None = None) -> bool:
    """Atomically deduct `cost` if the balance suffices; write the ledger row in
    the same transaction. Returns False (and leaves the balance untouched) otherwise."""
    updated = (
    ...
    if not updated:
        ...
        return False
    db.add(
        models.CreditTransaction(user_id=user.id, amount=-cost, kind="charge", endpoint=endpoint)
    )
    db.commit()
    db.refresh(user)
    return True
...
def refund_user(
    ...
    db.add(
        models.CreditTransaction(user_id=user.id, amount=amount, kind="refund", endpoint=endpoint)
    )
    db.commit()
    db.refresh(user)
    return user
...
def grant_credits(
    ...
    """Atomically add credits with a ledger row. Returns False when `external_id`
    was already processed, making event-driven crediting idempotent."""
    db.query(models.User).filter(models.User.id == user.id).update(
        {models.User.credits: models.User.credits + amount}
    )
    db.add(
...
def list_transactions(
    ...
    return query.order_by(models.CreditTransaction.id.desc()).offset(offset).limit(limit).all()


def list_users_with_stats(db: Session) -> list[dict]:
    """Per user: balance plus consumption and last-activity from the ledger."""
    consumed = func.sum(
        case(
            (models.CreditTransaction.kind == "charge", -models.CreditTransaction.amount),
            else_=0,
        )
    )
    rows = (
        ...
        .outerjoin(models.CreditTransaction, models.CreditTransaction.user_id == models.User.id)
...
def aggregate_stats(db: Session) -> dict:
    ...
    def _sum_for(kind: str, column) -> int:
        value = db.query(func.sum(column)).filter(models.CreditTransaction.kind == kind).scalar()
